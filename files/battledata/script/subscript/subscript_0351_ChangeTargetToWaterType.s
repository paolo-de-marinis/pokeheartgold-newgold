...
_PURE_TYPE:
    // Nor a target that is purely Water already, with no type added to it
    // (Pokemon Central, Inondazione); the reference's IsPureType, before the
    // move. The games show the move and then fail it; here it fails
    // before it is shown, as every other failure does.
    CompareMonDataToValue OPCODE_NEQ, BATTLER_CATEGORY_DEFENDER, BMON_DATA_TYPE_1, TYPE_WATER, _CHANGE_TYPE
    CompareMonDataToValue OPCODE_NEQ, BATTLER_CATEGORY_DEFENDER, BMON_DATA_TYPE_2, TYPE_WATER, _CHANGE_TYPE
