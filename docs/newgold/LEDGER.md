...
Implementation         ████████████████████████████████████████████████░░  95%
Verified in play       ██████░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░  12%
Audit rows closed      ████████████████████████████████████░░░░░░░░░░░░░░  72%

Audit: 48 of 170 rows in docs/newgold/AUDIT-*.md still open.
...
Trainers    ██████████████████████████████████████████████████   738 /  738
Tests       95 files
ROM         162.8 MB of 268.4 MB   (2G card, 61% used)
```
<!-- LEDGER:COUNTS:END -->
...
| Abilities | 278 do something — 123 retail effects and 155 added; of the other 41, sixteen are a name in the reference too, and the rest want a primal weather, a paradox booster or an in-battle form change -- Schooling and Power Construct among them since the forms became species, which `test_ability_effects.py` now counts as pending rather than done | 🟠 278 / 319 |
| Level-up learnsets | 1042; the entry is a word now, so a move past 511 can be learnt | ✅ done |
| Battle script commands | 294 opcodes, name- and number-identical to the reference. Every command a script here runs does what the reference's does: the last two that only read their operands -- SetMoveConditionFlag, which is Powder, Laser Focus, Glaive Rush and Throat Chop, and ChangeExecutionOrderPriority, which is After You and Quash -- got their state, their consumers and, for the order, the reference's re-sort of whoever has not moved yet after every move. 18 added handlers are still stubs -- CheckProtectContactMoves is done now, the six shields answering a touch -- and `test_battle_commands.py` names each one and proves no script runs it: they wait on mega and ultra burst, tera, totems, primal weather, Parental Bond and the batched messages of a spread move, none of which this tree has | 🟠 276 / 294 |
...
| Effects with a script but no C | 22 effects the reference's C reads have a script here and no C, each named with its reason in `test_move_effects.py`'s UNREAD_HERE -- Shed Tail's switch, Quash through a substitute and the others the audit's move rows list. The row said none until 2026-09-23: it counted only the effects with no script at all. Techno Blast and Multi-Attack, the last of those, read a held Drive or Memory as they should | 🟠 partial |
...
| The species in play | all 1438 species and forms: sprite, icon, cry, name, Dex, an ability that does something | 🔴 never |
