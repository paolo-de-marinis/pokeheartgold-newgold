...
Implementation         ██████████████████████████████████████████████████  99%
Verified in play       ██████░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░  13%
Audit rows closed      ██████████████████████████████████████████████░░░░  91%

Audit: 41 of 441 rows in docs/newgold/AUDIT-*.md still open.
...
| Item restoration, friendship effects, AI item grab | `RESTORE_ITEMS_AT_BATTLE_END`, `FRIENDSHIP_EFFECTS`, `AI_CAN_GRAB_ITEMS`. Since the ninth round the restoration gives back a Berry a Magician or Pickpocket took, leaves a caught wild Pokemon the item taken from it and no longer wipes what Pickup and Honey Gather find (9c3290462, d8435ac78, c966f119d): the reference skips any Berry and has Pickup's order, neither copied | ✅ done |
...
| Widened Pokédex and footprints | footprints and Dex text for all 1041: entry, category, height and weight in twenty-seven banks; and since the eighth round an area record for every Dex species: `dex_areas.py` rebuilds retail's 493 byte for byte and writes the rest (5fff64f57), and 22 of the added species show where New Gold's wild tables put them (264ee2f01), without swarms, the Safari Zone or the Bug-Catching Contest, as retail's records | ✅ done |
...
| Moves | 131 effect scripts, 59 subscripts, 62 side-effect table slots, and the three "X used Y!" lines each move reads from bank 3. Until 2026-09-23 that bank stopped at Shadow Force, so any move past 467 stopped the battle the moment it was used -- Morty's Misdreavus found it with Hex. All 905 in scope do something: the seventh round gave 54 of the moves the engine leaves unimplemented their effect, the eighth Ally Switch and the three Pledges, and the ninth Sky Drop (0431d6c0a) and Revival Blessing (9752fcd0f, after the battle party menu's switch checks were decompiled matching in 45f889314), both seen in the headless harness: Pidgeot lifting the opposing Furret and dropping it the next turn, Heracross refused in the party menu and Machamp revived. The 18 Max moves are Dynamax's, out of scope and out of the count | ✅ 905 / 905 |
| Abilities | all 318 do something — 123 retail effects and 195 added. The last 31 came in the fifth round, each from its Pokémon Central page where the reference leaves it empty (Paolo, 2026-09-23: none stays empty): the primal weathers, Parental Bond, Illusion, Emergency Exit and Wimp Out, Dancer, the Terapagos and Ogerpon abilities -- the Tera forms being species, they act as the form comes in -- Mega Sol, Shields Down, Battle Bond and the ally and switch-in abilities. The 319th constant, TEMP4, is a slot the engine reserved and named "Placeholder", carried by no species, and `test_ability_effects.py` counts it apart. `PENDING` is empty since the eighth round wrote Commander (981ec26bd, after 28f300daa): in a double battle Tatsugiri goes into Dondozo's mouth, out of every move's reach, Dondozo's five stats rise two stages, neither can leave or be forced out, and Order Up raises a stat by Tatsugiri's form (000e5e360). It had stood there since the seventh round, when a block list stopped counting as an ability's reading (acc7c51ae); Gulp Missile and Poison Puppeteer, which had passed the same way, do what Pokemon Central says (a76cef294, 66281a56e). None of the 31 has been seen on an emulator: the evidence is host tests and the built scripts. Commander has, in the ninth round, in melonDS on a display of its own (`nested.py`): Tatsugiri swallowed and hidden with its health box kept, Dondozo's stats raised, only Dondozo asked for a move, and a Mach Punch aimed at Tatsugiri missing | ✅ 318 / 318 |
...
| Items | 2685 of 2685: every item konefr's tree defines has a counterpart here. They are imported by name and renumbered densely from where this tree was -- their Black Augurite is 1691 and this one's is 537, because taking their ids would renumber every item in Paolo's save, his bag and every held item in the data -- and `tools/newgold/import/item_map.csv` is the mapping, 2685 of their names against 2685 ids here. This pass brought 2129 of them: ITEMS_COUNT 564 to 2693, the eight over their range being the slots HeartGold left empty that konefr filled with real items, so the gap stays here and their item is numbered beside it. 518 icons are built from konefr's PNGs, resolved by archive member and never by name; the other 1611 take ITEM_NONE's question mark knowingly, because konefr's art for them is byte-identical to `none.png`. 84 of their constants are reserved gaps with no name in 222.txt and are written the way this game already writes an empty slot. Descriptions are konefr's own single line, "Custom item description", for everything but the eighteen the pass before wrote prose for -- their description bank is that one line for the whole game. The 556 records both trees share now carry konefr's numbers rather than Game Freak's -- 277 prices, an Amulet Coin among them at 30000 against 100, and all 64 of Natural Gift's sixth-generation powers; Prism Scale keeps this engine's party-use evolution instead of the reference's hold effect, which is the one disagreement left and it is deliberate. Since the fifth round every one of the 64 hold effects the range brought is read -- where they fall short of the games is an audit row -- and the twenty-one Mints, the Ability Capsule and the Ability Patch work from the party menu, the four Nectars, the Reveal Glass and the Rotom Catalog from the bag. The DNA Splicers fuse and separate Kyurem, keeping the fused Reshiram or Zekrom in the save's extension as konefr's config does (36a24a778). What is not true yet: nothing puts any of the 2129 in the world -- no mart, no hidden item, no wild Pokemon carrying one beyond the nine already imported | ✅ 2685 / 2685 |
| Evolutions | 496 species evolve: every line the reference draws between two species this tree names, the forms included now that they are species -- Kubfu takes the Scroll of Waters to Rapid Strike Urshifu, Galarian Slowpoke the Galarica Wreath to Galarian Slowking, the Antique Sinistea and Masterpiece Poltchageist take the chipped pot, and a form that evolves keeps its form, an Antique Sinistea becoming an Antique Polteageist as the reference carries the number over. The sixth round brought the methods that were missing (the magnetic field, a level by the time of day, in the rain, with a Dark type in the party, by nature, a move used twenty times, three critical hits, damage taken, a thousand steps walked out of the ball, Gimmighoul's coins, the Bisharp a Bisharp defeats, Karrablast traded for Shelmet), and the eighth Paolo's designs (2026-09-23), beyond konefr's, in the New Gold layer: Milcery, whose spin the reference never checks and whose forms it numbers wrongly, evolves levelling up anywhere in the Ice Path holding one of seven Berries, which picks the Alcremie and is used up (c12e110f2); and, no Leader's Crest or Gimmighoul Coin being placed, Bisharp also evolves levelling up knowing Swords Dance (8421a5de5), and Gimmighoul, both forms, learns Pay Day at 55 and evolves knowing it (fe832350f, ba7300604). A female-form target (Unfezant, Pyroar) is a species of its own here that the Dex counts as its base. The seventh round added the Moss Rock and the Ice Rock (196be0087) and Karrablast and Shelmet evolving in a GTS trade (3e8629c64). `import_evolutions.py` reports every row it cannot express. hg-engine's own changes to eight of HeartGold's species are in as well, read at `d0380a487` with `--engine-reworks`: the Linking Cord for Kadabra, Machoke, Graveler and Haunter, the Thunder Stone for Magneton and Nosepass, the Leaf and Ice Stones for Eevee, a Prism Scale trade for Feebas. Until 2026-09-23 they were left out as a rework rather than a change, and Magnezone, Probopass, Leafeon and Glaceon could not be had: their retail rows name Mt. Coronet, Eterna and Route 217 | ✅ done |
...
| A battle renders | forced against species 19, 575, 900 and 1041, indoors and on Route 29's grass: background, sprite, name, and the battle runs to its end. Since the ninth round the tools watch battles too. `tools/newgold/devkit/diag/nested.py` (2ce7383bd) runs melonDS in a nested KWin with its own display, D-Bus and home, touching nothing on the desktop; a trainer double battle in Azalea Gym showed there the Pledges combining and the sea of fire burning at the turn's end, Ally Switch's mosaic and swap, and Commander's hiding, and caught Ally Switch playing Hypnosis's animation after its line (366209937). And the headless harness draws them now, where the screen was black from overlay 12 on: `core.py` showed Sky Drop's lift and drop and Revival Blessing's party menu, and, after a catch, the HP box leaving before the capture experience (9f6bb064b) | ✅ done |
...
| The species in play | all 1438 species and forms: sprite, icon, cry, name, Dex, an ability that does something. `species.py` (0365359e6 and 0395ef9b2) walked them in the seventh round, 1488 species and forms and 3671 variants -- every gender, every ability -- through the PC and its icons, the summary, the Dex's 1025 pages and a battle each, and 51 more battles for the abilities only a second or hidden slot carries. What it found the round fixed: Surge battles hanging, 18 cries that never played, the Normal second type undrawn, the summary's cry asking for the ability as the form, savedit's Dex; the 16 striped icons it flagged are fixed by 6c59633e0 and ee3154572, four of them seen since in the Dex grid. What it could not see is in the audit: a battle was judged by its markers, the harness then drawing none, with the wild Pokemon always form 0, and the walk has not been rerun since the harness draws battles; the Dex's FORMS page, walked since the eighth round with its AREA pages and the summary's Info page (3a51b33e5), takes every form a species keeps since the ninth (e5f43b51a, b03b099dc: 13 species and 62 entries, no failure); nobody listened to the cries | ✅ seen |
| Automated tests | All but one read source and data, `test_boot.py` runs both ROMs and both diagnostics builds (5b4d3b451). Each memory bug got its regression test after the fact, not before, and the two that caught something before it shipped are the newest: the nickname lines that come in threes, and the stubs a script must not run | 🟠 partial |
