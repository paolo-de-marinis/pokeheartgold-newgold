...
| Cries and audio expansion / executable logic and resource | Cry pseudobanks and `NNSi_SndArcLoadBank_hook` | `lib/NitroSystem/src/sndarc_loader.c` (`NNSi_SndArcLoadBank`, matching C, then a number with a wave archive and no bank played on bank 1's instrument), `src/unk_02005D10.c` (`PlayCry`, `PlayCryEx`), `tools/newgold/import/import_cries.py` and `sdat.py`, C | VANILLA C DECOMPILED; MATCHING; PORTED; BUILDS; VERIFIED (in play) | `test_cries` (the tables' cost in the sound heap, the bankless numbers), `test_boot` (the intro's and the title's music, the title's cry), scenarios `cry_added_species` and `continue_route29` |
