...
class SpeciesRecordTests(unittest.TestCase):
    ...
    def test_a_move_learnt_by_level_up_can_be_taught_by_its_machine(self):
        ...
        Wave (TM34), and konefr's Delibird Roost (TM51). Checked against this
        game's own level-up archive, for every species."""
        import wotbl
        ...
        for index, record in enumerate(self.records):
            for entry in wotbl.decode(learnsets[index]):
                ...
                if field and number not in record[field]:
                    missing.append(f"{record['species']} {field} {number}")
