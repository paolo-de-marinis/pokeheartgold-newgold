#!/usr/bin/env python3
"""Run the photo viewer's party count with host sanitizers.

Photo_CountValidMons (src/application/view_photo.c) chooses between the
single-Pokemon caption and the "... and friends" one. It is extracted and run
against the real PhotoMon layout; the rest of the photo is a stand-in.
"""
...
int main(void) {
    ...
    assert(Photo_CountValidMons(&photo) == 2);
    ...
    assert(Photo_CountValidMons(&added) == 2);
    ...
    assert(Photo_CountValidMons(&empty) == 0);
    return 0;
}
'''
...
class PhotoCaptionTests(unittest.TestCase):
    def test_added_species_count_toward_the_caption(self):
        ...
        native = function((ROOT / "src/application/view_photo.c").read_text(), "Photo_CountValidMons")
        with tempfile.TemporaryDirectory(prefix="newgold-photo-caption-") as temp:
            c, exe = Path(temp) / "check.c", Path(temp) / "check"
            c.write_text(PREFIX.replace("@PARTY_SIZE@", party).replace("@PHOTOMON@", photomon).replace("@NATIVE@", native))
