...
void PartyMenu_LearnMoveToSlot(PartyMenu *partyMenu, Pokemon *mon, int moveIdx) {
    ...
    if (partyMenu->args->itemId != ITEM_NONE) {
        // New Gold's TMs and HMs are reusable, so teaching from one leaves it
        // in the bag. A TR is spent, as in Sword and Shield and in hg-engine's
        // PartyMenu_LearnMoveToSlot. This asks about the item because HMs are
        // forgettable now and MoveIsHM no longer says which moves came from a
        // machine.
        if (!ItemIsMachine(partyMenu->args->itemId) || ItemIsTR(partyMenu->args->itemId)) {
            Bag_TakeItem(partyMenu->args->bag, partyMenu->args->itemId, 1, HEAP_ID_PARTY_MENU);
        }
