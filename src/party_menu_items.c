...
void PartyMenu_LearnMoveToSlot(PartyMenu *partyMenu, Pokemon *mon, int moveIdx) {
    ...
    if (partyMenu->args->itemId != ITEM_NONE) {
        // New Gold machines are reusable, so nothing taught from one leaves
        // the bag. This asks about the item because HMs are forgettable now
        // and MoveIsHM no longer says which moves came from a machine.
        if (!ItemIsMachine(partyMenu->args->itemId)) {
            Bag_TakeItem(partyMenu->args->bag, partyMenu->args->itemId, 1, HEAP_ID_PARTY_MENU);
        }
