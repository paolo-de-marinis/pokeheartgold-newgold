...
// The reference ships BLOCK_LEARNING_UNIMPLEMENTED_MOVES on: a move it never
// implemented is left out of every level-up learnset and every trainer's
// moveset, and refused in battle (IsMoveUnimplemented, src/moves.c at
// d0380a487).
BOOL IsMoveUnimplemented(u16 moveId) {
    return (GetMoveAttr(moveId, MOVEATTR_UNK9) & MOVE_FLAG_UNUSABLE_UNIMPLEMENTED) != 0;
}
