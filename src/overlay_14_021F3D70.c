...
// only when its second type is not its first. Retail hid a Normal second
// type too, as no species of its had one; Litleo and Pyroar are Fire and
// Normal, and the reference shows it (bytereplacement, "normal as a second
// type should show up in the dex/pc"). The fourth argument of each call is a
// resource id ov14_021F3D0C never reads.
void ov14_021F3D70(PCBoxAppGraphics *graphics, PCBoxDisplayMon *mon) {
    if (mon->isEgg) {
