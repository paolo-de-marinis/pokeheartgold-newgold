...
// which holds the five conditions after the types; 18 is retail's count of
// types. This game never shows them: the pages switch to contest mode only
// when the high nibble of the byte at 0x2077 is set (Platinum's
...
void ov08_02221328(BattleParty *battleParty) {
    ...
    for (u16 i = 0; i < MAX_MON_MOVES; i++) {
        ...
        ov08_02220AEC(battleParty, battleParty->sprites[MOVE_TYPES_SPRITES_OFFSET + i], MOVE_TYPES_CHAR_RESOURCE_ID_OFFSET + i, GetMoveAttr(pokemon->moves[i].move, MOVEATTR_CONTEST_TYPE) + 18);
    ...
    if (battleParty->context->moveToLearn != MOVE_NONE) {
        ov08_02220AEC(battleParty, battleParty->sprites[MOVE_TO_LEARN_TYPE_SPRITE_INDEX], MOVE_TYPES_CHAR_RESOURCE_ID_OFFSET + MAX_MON_MOVES, GetMoveAttr(battleParty->context->moveToLearn, MOVEATTR_CONTEST_TYPE) + 18);
